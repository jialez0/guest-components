...
pushd guest-components/dist/tpm-deps/anolis_7
# Extract tpm2-tss RPM package
mkdir -p tpm2-tss-extracted
pushd tpm2-tss-extracted
rpm2cpio ../tpm2-tss-2.4.6-1.an7.x86_64.rpm | cpio -idmv
popd
popd

%build
mkdir -p ./guest-components && pushd guest-components
cargo build -p attestation-agent --bin ttrpc-aa --release --no-default-features --features bin,ttrpc,rust-crypto,coco_as,kbs,tpm-attester,csv-attester,instance_info --target x86_64-unknown-linux-gnu
...
pushd dist/tpm-deps/anolis_7/tpm2-tss-extracted
...
- Integrated tpm2-tss version 2.4.6 from pre-built RPM package for Anolis 7 